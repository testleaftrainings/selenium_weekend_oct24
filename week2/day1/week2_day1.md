...
## Steps to Locate Element in the Webpage
-----------------------------------------
   1. Right on the Element which you want to Locate
   2. Click on the inspect option 
   3. The Element will be availble in Elements


   or
   1. click on the three dots icon from the top right --> click on the more tools --> click on the developer tools (shortcut -- ctrl + shift + i -> it helps open and close the DOM)
   2. Click on the spyTool(dotted squarebox with Arrow)/(Shortcut ctrl+shit+c --> helps to open the dom and enable the spyTool) and click on the Element 
